...
interface SiteEditorProps {
  siteId: string;
  currentName: string;
  onSave: (siteId: string, newName: string) => void;
  onClose: () => void;
}

const SiteEditor: React.FC<SiteEditorProps> = ({ siteId, currentName, onSave, onClose }) => {
    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-1">
            ...
            <input 
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-sm text-white focus:ring-2 focus:ring-blue-500/50 outline-none"
        ...
        <div className="p-6 bg-slate-800/30 border-t border-slate-800 flex gap-3">
          ...
          <button 
            onClick={() => onSave(siteId, name)}
            className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg transition-colors shadow-lg shadow-blue-500/20"
          >
            GUARDAR CAMBIOS
          </button>
        </div>
